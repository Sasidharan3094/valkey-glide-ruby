  ...
  module Commands
    module StreamCommands
      ...
      def xinfo(subcommand, key, group = nil)
        send("xinfo_#{subcommand.downcase}", key, group)
      end

      def xinfo_consumers(key, group)
        send_command(RequestType::X_INFO_CONSUMERS, [key, group].compact)
      end

      def xinfo_groups(key, group)
        send_command(RequestType::X_INFO_GROUPS, [key, group].compact)
      end

      def xinfo_stream(key, group)
        send_command(RequestType::X_INFO_STREAM, [key, group].compact)
      end
      ...
      # @return [Integer] the number of entries actually deleted
      def xdel(key, *ids)
        raise TypeError, "Expected String for key, got #{key.class}" unless key.is_a?(String)
        ids.flatten.each do |id|
          raise TypeError, "Expected Array of String for ids, got #{id.class}" unless id.is_a?(String)
        end

        if ids.empty?
          ids = ['0-0'] # default value for xdel command
        elsif ids.size == 1 && ids.first.is_a?(Array)
          ids = ids.first # splatted array
        else
          ids = ids.flatten # splatted arguments
        end

        raise CommandError, "No ids provided for xdel command" if ids.empty?
      ...
      def xgroup(subcommand, key, group, id_or_consumer = nil, mkstream: false)
        send("xgroup_#{subcommand.to_s.downcase}", key, group, id_or_consumer, mkstream: mkstream)
      end

      def xgroup_create(key, group, id_or_consumer = nil, mkstream: false)
        args = [key, group, id_or_consumer, (mkstream ? 'MKSTREAM' : nil)].compact
        send_command(RequestType::X_GROUP_CREATE, args)
      end

      def xgroup_create_consumer(key, group, id_or_consumer = nil, mkstream: false)
        args = [key, group, id_or_consumer, (mkstream ? 'MKSTREAM' : nil)].compact
        send_command(RequestType::X_GROUP_CREATE_CONSUMER, args)
      end

      def xgroup_del_consumer(key, group, id_or_consumer = nil, mkstream: false)
        args = [key, group, id_or_consumer, (mkstream ? 'MKSTREAM' : nil)].compact
        send_command(RequestType::X_GROUP_DEL_CONSUMER, args)
      end
      ...
      def xack(key, group, *ids)
        raise TypeError, "Expected String for key, got #{key.class}" unless key.is_a?(String)
        raise TypeError, "Expected String for group, got #{group.class}" unless group.is_a?(String)
      ...
      #   valkey.xclaim('mystream', 'mygroup', 'consumer1', 3600000, '0-0', count: 50)
      # @example Claim next pending message stuck > 5 minutes and don't mark as retry
      #   valkey.xclaim('mystream', 'mygroup', 'consumer1', 3600000, '0-0', justid: true)
      # @example Claim next pending message after this id stuck > 5 minutes  and mark as retry
      #   valkey.xautoclaim('mystream', 'mygroup', 'consumer1', 3600000, '1641321233-0')
      #
      # @param key           [String]        the stream key
      # @param group         [String]        the consumer group name
      # @param consumer      [String]        the consumer name
      ...
      def xpending(key, group, *args, idle: nil)
        ...
        #
        send_command(RequestType::X_PENDING, command_args)
      end

      private

      def _xread(cmd, args, keys, ids, blocking_timeout_msec)
        ...
        send_command(cmd, args, &Utils::HashifyStreamEntries)
